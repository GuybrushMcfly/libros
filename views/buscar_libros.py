...
def buscar_libros():
    ...
    tipo_busqueda = st.radio(
        "Buscar por:", 
        options=["Autor", "Título", "Editorial (próximamente)"],
        horizontal=True
    )
    ...
    if tipo_busqueda == "Autor":
        ...
        if autor_seleccionado != "- Seleccioná autor -":
            ...
            if libro_ids:
                ...
                df_base["nombre_formal"] = df_base["nombre_formal"].fillna("-")
        ...
        texto_busqueda = st.text_input("Título contiene...", value="", key="titulo_busqueda")
    
        # Solo ejecuta la búsqueda si hay al menos 2 caracteres
        if texto_busqueda and len(texto_busqueda.strip()) >= 2:
        ...
        elif texto_busqueda and len(texto_busqueda.strip()) < 2:
            st.info("Ingresá al menos 2 letras para buscar por título.")
        else:
